...
function getConnection()
{
	...
	$data['_dbname'] = (string) $connection->dbname;
...
function getDirectLinkInfo()
{
    if (isset($_SERVER['HTTP_REFERER'])) {
        ...
        // query direct link
        $direclinkTable = getTable('affiliateplus_direct_link');
        $direclinkDomainTable = getTable('affiliateplus_direct_link_domain');
        
        if(!tableExist($direclinkTable) || !tableExist($direclinkDomainTable)) {
            return null;
        }
        
        $sql = "SELECT `direct_link_id`, `banner_id`, `$direclinkDomainTable`.`account_id` FROM `$direclinkTable` ";
        $sql .= "JOIN `$direclinkDomainTable` ON `$direclinkTable`.`domain_id` = `$direclinkDomainTable`.`domain_id` ";
        $sql .= "WHERE `$direclinkDomainTable`.`domain_id` = '$domain' ";
        $sql .= "AND `$direclinkTable`.`source_page` = '$path' ";
        $sql .= "AND `$direclinkTable`.`status` = '2' ";
        $sql .= "AND `$direclinkDomainTable`.`status` = '2' ";
...
{
    global $data;
    $sql = "SELECT COUNT(*) as total FROM information_schema.tables WHERE table_schema = '{$data['_dbname']}'  AND table_name = '{$table}'";
